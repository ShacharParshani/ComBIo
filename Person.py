def probability_to_spread(s_level):  # Probability of spreading the rumor
    if s_level == 1 | s_level == 0:  # 0 for the case that the level of skepticism temporarily decreased by one
        return 1
    if s_level == 2:
        return 2/3
    if s_level == 3:
...
class Person:
    def _init_(self, s):
        ...
        self.prevent = 0  # 0 if the person is not prevented from spreading the rumor, otherwise -
